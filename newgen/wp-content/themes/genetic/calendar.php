...
function replacenewline($string)
{
	...
	return $newstr;
				...
				$eventQuery = "SELECT * FROM wp_calendar WHERE event_begin >='".date('Y-m-d')."'   ORDER BY event_begin asc, event_time  LIMIT 6";
...

				{
			 		...
			 		while($row1 = mysql_fetch_array($eventArray))
					{
						...
						$print_time = date("g:i A", $mk_date);
								...
								$str .= '<h2  id="event_id_'.$row1['event_id'].'">'.$row1['event_title'].'</h2>
								<p  style="font-size:12px;">
								'.$print_date;
								if($row1["event_end"] !="" && $row1["event_end"]!=$row1["event_begin"]) 
								{
									$str .= " - ".$end_print_date;
								}
								$str .= '</p>';	
								if($row1['event_time']!="23:59:59")
								{
									$str .= '<p  style="font-size:12px;">'.$print_time.'</p>';
								}
								$str .= '
								<p style="padding:5px 0;">'.$row1['event_desc'].'</p>
								<p >
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
								  <tr>
									<td width="14%">Location :</td>
									<td width="86%">'.replacenewline(htmlspecialchars(stripslashes($row1['event_location']))).'</td>
								  </tr>
								</table>		
								</p>';
