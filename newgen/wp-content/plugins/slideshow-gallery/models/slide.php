		...
		if (!empty($data)) {
			...
			elseif ($type == "file") {
				if (!empty($image_oldfile) && empty($_FILES['image_file']['name'])) {
					...
					$pastwork = $imagepath . $name . '-pastwork.' . $ext;
					if(!file_exists($pastwork))
					 {
							image_resize($imagefull, $width = 600, $height = 400, $crop = true, $append = 'pastwork', $dest = null, $quality = 100);
							@chmod($homefull, 0777);							
					 }
					
					
					if(isset($_POST["Slide"]["show_at_home"]))
					{
					   $homefull = $imagepath . $name . '-home.' . $ext;
					   if(!file_exists($homefull))
					   {
					   
					   		list($org_width, $org_height, $type, $attr) = getimagesize($imagefull);
							$newheight =490;
							if($org_height < 490)
							{
								$newheight = $org_height-2;							
							}
							
							
							image_resize($imagefull, $width = 0, $height = $newheight, $crop = true, $append = 'home', $dest = null, $quality = 100);
							@chmod($homefull, 0777);
					...
					if ($_FILES['image_file']['error'] <= 0) {
						...
						elseif (!move_uploaded_file($_FILES['image_file']['tmp_name'], $imagefull)) { $this -> errors['image_file'] = __('Image could not be moved from TMP to "wp-content/uploads/", please check permissions', $this -> plugin_name); }
							...
							$squarefull = $imagepath . $name . '-square.' . $ext;
							$homefull = $imagepath . $name . '-home.' . $ext;
							$pastwork = $imagepath . $name . '-pastwork.' . $ext;
							...
							image_resize($imagefull, $width = 160, $height = 160, $crop = true, $append = 'square', $dest = null, $quality = 100);
							image_resize($imagefull, $width = 600, $height = 400, $crop = true, $append = 'pastwork', $dest = null, $quality = 100);
				...
				if (empty($image_url)) { $this -> errors['image_url'] = __('Please specify an image', $this -> plugin_name); }
					...
					if ($image = wp_remote_fopen($image_url)) {
						...
						if (!file_exists($filefull)) {
							...
							$squarefull = $filepath . $name . '-square.' . $ext;
							$homefull = $filepath . $name . '-home.' . $ext;
							$pastwork = $filepath . $name . '-pastwork.' . $ext;
							...
							image_resize($filefull, $width = 160, $height = 160, $crop = true, $append = 'square', $dest = null, $quality = 100);
							image_resize($filefull, $width = 600, $height = 400, $crop = true, $append = 'pastwork', $dest = null, $quality = 100);
