	...
	if ( !empty($events) )
	{
		...
		foreach ( $events as $event )
		{
			...
			<tr class="<?php echo $class; ?>">
				...
				<td><?php if ($event->event_time == '00:00:00' || $event->event_time == '23:59:59') { echo __('N/A','calendar'); } else { echo stripslashes($event->event_time); } ?></td>
				<td>
				<?php 
		...
		<div id="linkadvanceddiv" class="postbox">
			<div style="float: left; width: 98%; clear: both;" class="inside">
				...
				<td><textarea name="event_desc" class="input" rows="5" cols="50"><?php if ( !empty($data) ) echo htmlspecialchars(stripslashes($data->event_desc)); ?></textarea></td>
                                ...
                                <tr>
				<td style="vertical-align:top;"><legend><?php _e('Event Location','calendar'); ?></legend></td>
				<td><textarea name="event_location" class="input" rows="5" cols="50"><?php if ( !empty($data) ) echo htmlspecialchars(stripslashes($data->event_location)); ?></textarea></td>
                                </tr>
                                <tr>
...
if ( $action == 'add' )
{
	...
	$event_location = !empty($_REQUEST['event_location']) ? $_REQUEST['event_location'] : '';
	$begin = !empty($_REQUEST['event_begin']) ? $_REQUEST['event_begin'] : '';
	$end = !empty($_REQUEST['event_end']) ? $_REQUEST['event_end'] : '';
	$time = !empty($_REQUEST['event_time']) ? $_REQUEST['event_time'] : '23:59:59';
	...
	$time_format_two = '/^([2][0-3]):([0-5][0-9])$/';
        ...
        if (preg_match($time_format_one,$time) || preg_match($time_format_two,$time) || $time == '23:59:59'  || $time == '')
          {
            $time_ok = 1;
			if ($time == '')
			  {
			$time_to_use = '23:59:59';
			  }
			else if ($time == '00:00')
			  {
	...
	if (preg_match('/^.{1,30}$/',$title))
	  ...
	  {
              ?>
              <div class="error"><p><strong><?php _e('Error','calendar'); ?>:</strong> <?php _e('The event location must be atleast 5 character in length','calendar'); ?></p></div>
              <?php
	  }
...
//	  echo $start_date_ok  ."::". $end_date_ok  ."::". $time_ok  ."::". $url_ok  ."::". $title_ok  ."::". $recurring_ok."::".$event_location;
	if ($start_date_ok == 1 && $end_date_ok == 1 && $time_ok == 1 && $url_ok == 1 && $title_ok == 1 && $recurring_ok == 1  && $event_location_ok == 1)
	  {
	    $sql = "INSERT INTO " . WP_CALENDAR_TABLE . " SET 
		event_title='" . mysql_escape_string($title)
	     . "', event_desc='" . mysql_escape_string($desc) . "',event_location='" . mysql_escape_string($event_location) . "', event_begin='" . mysql_escape_string($begin) 
		...
		. " AND event_desc='" . mysql_escape_string($desc) . "' AND event_location='" . mysql_escape_string($event_location) . "' AND event_begin='" . mysql_escape_string($begin) . "' AND event_end='" . mysql_escape_string($end) . "' AND event_recur='" . mysql_escape_string($recur) . "' AND event_repeats='" . mysql_escape_string($repeats) . "' LIMIT 1";
	...
	else
	  {
		...
		$users_entries->event_location = $event_location;
