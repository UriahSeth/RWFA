<?php
	  ...
	  */	   
      ...
      function get_next_page_nav($user_link="",$link_para="",$rewrite_on=false)
      {
		if($this->total_page<=1)
			return;
	  	if(trim($user_link)=="")
        ...
        if(!$this->is_last_page()&& $this->show_prev_next)
		{
		   ...
		   if($rewrite_on)
		   {
		   		echo '<span class="pagelinkspan1 nextpagelink"><a  href="'.$this->qry_str.($this->page+1).'"  onmouseout="MM_swapImgRestore();" onmouseover="MM_swapImage(\'grey_next\',\'\',\''.get_bloginfo("template_directory").'/images/orange_next.gif\',1);"><img name="grey_next" src="'.get_bloginfo("template_directory").'/images/grey_next.gif" width="24" height="41" border="0" id="grey_next" alt="next" /></a></span>';
			}
			else
			{
      ...
      function get_prev_page_nav($user_link="",$link_para="",$rewrite_on=false)
      {
		if($this->total_page<=1)
			return;
	  	if(trim($user_link)=="")
        ...
        if(!$this->is_first_page()&& $this->show_prev_next)
		{
			...
			if($rewrite_on)
		   {
				echo '<span class="pagelinkspan1 prevpagelink"><a href="'.$this->qry_str.($this->page-1).'"  onmouseout="MM_swapImgRestore();" onmouseover="MM_swapImage(\'grey_prev\',\'\',\''.get_bloginfo("template_directory").'/images/orange_prev.gif\',1);"><img name="grey_prev" src="'.get_bloginfo("template_directory").'/images/grey_prev.gif" width="24" height="41" border="0" id="grey_prev" alt="prev" /></a></span>';
			}
			else
			{
      ...
      function get_scroll_prev_page_nav($user_link="",$link_para="",$rewrite_on=false)
      {
        ...
        if($this->page>$this->scroll_page &&$this->show_scroll_prev_next)
		{
         	if(!$rewrite_on)
			{
			 ...
			 {
			 	echo '<span class="pagelinkspan1 prevpagelink"> <a href="'.$this->qry_str.($this->page-$this->scroll_page).'" '.$link_para.'class="pagelink">'.$user_link.'</a></span> ';
			 }
      ...
      function get_scroll_next_page_nav($user_link="",$link_para="",$rewrite_on=false)
      {
		if($this->scroll_page>=$this->total_page)
			return;
	  	if(trim($user_link)=="")
        ...
        if($this->total_page>$this->page+$this->scroll_page &&$this->show_scroll_prev_next)
		{
			...
			{
				echo '<span class="pagelinkspan1 nextpagelink"> <a href="'.$this->qry_str.($this->page+$this->scroll_page).'" '.$link_para.' class="pagelink">'.$user_link.'</a></span> ';
			}	
		}	
        elseif($this->show_scroll_prev_next && $this->show_disabled_links)
            echo $user_link;
	  ...
	  function get_page_nav_next($rewrite_on=false)
      {
	  	if($this->total_records<=0)
			return false;
	  	if($this->total_records<=$this->records_per_page)
        ...
        $this->get_next_page_nav("",$this->link_para,$rewrite_on);
        $this->get_scroll_next_page_nav("",$this->link_para,$rewrite_on);
      
      }
	  function get_page_nav_prev($rewrite_on=false)
      {
	  	if($this->total_records<=0)
			return false;
	  	if($this->total_records<=$this->records_per_page)
        ...
        $this->get_scroll_prev_page_nav("",$this->link_para,$rewrite_on);
        $this->get_prev_page_nav("",$this->link_para,$rewrite_on);
